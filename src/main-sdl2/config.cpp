...
template <class T> std::optional<std::reference_wrapper<const T>> json_get(const picojson::value &v)
{
    if (!v.is<T>())
        return std::nullopt;
    return v.get<T>();
...
template <class T> std::optional<std::reference_wrapper<const T>> json_object_get(const picojson::object &o, const std::string &key)
{
    using std::end;
...
picojson::value json_value_int(const int x) { return picojson::value(double(x)); }
